...
public class DDSImage
    {
        ...
        private void Parse(byte[] reader)
        {
            DDSStruct header = this.ReadHeader(reader);
        TextureFormat pixelFormat;
            ...
            if (header!=null)
            {
                ...
                if (data != null)
                {
                  m_bitmap= new Texture2D((int)header.width, (int)header.height, pixelFormat, false, true); //TODO
                  m_bitmap.LoadRawTextureData(data);
                  m_bitmap.Apply();
                }
            }
        }

        private byte[] ReadData(byte[] ddsBytes, DDSStruct header)
        {
            byte[] compdata = null;
            uint compsize = 0;
...
/*        if ((header.flags & DDSD_LINEARSIZE) > 1)
        ...
        private DDSStruct ReadHeader(byte[] ddsBytes)
        {
            DDSStruct header = new DDSStruct();
            int rr = 4;
            
            //byte[] signature = ddsBytes.ReadBytes(4);
            //if (!(signature[0] == 'D' && signature[1] == 'D' && signature[2] == 'S' && signature[3] == ' '))
              //  return false;

            header.size = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            if (header.size != 124)
                return null;

            //convert the data
            header.flags = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.height = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.width = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.sizeorpitch = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.depth = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.mipmapcount = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.alphabitdepth = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;

            header.reserved = new uint[10];
            for (int i = 0; i < 10; i++)
            {
                header.reserved[i] = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            }

            //pixelfromat
            header.pixelformat.size = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.flags = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.fourcc = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.rgbbitcount = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.rbitmask = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.gbitmask = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.bbitmask = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.pixelformat.alphabitmask = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;

            //caps
            header.ddscaps.caps1 = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.ddscaps.caps2 = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.ddscaps.caps3 = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.ddscaps.caps4 = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;
            header.texturestage = BitConverter.ToUInt32(ddsBytes, rr); rr += 4;

            return header;
        }

        private TextureFormat GetFormat(DDSStruct header, ref uint blocksize)
        {
        ...
        // iCompFormatToBpp
        private uint PixelFormatToBpp(int pf, uint rgbbitcount)
        {
            switch (pf)
            {
        ...
        // iCompFormatToBpc
        private uint PixelFormatToBpc(int pf)
        {
            switch (pf)
            {
                case PixelFormat.R16F:
        ...
        private void DxtcReadColors(byte[] data, ref Colour8888[] op)
        {
        ...
        private void DxtcReadColors(byte[] data, ref Colour565 color_0, ref Colour565 color_1)
        {
    ...
    /*
        private void ConvFloat16ToFloat32(uint* dest, ushort* src, uint size)
        {
            uint i;
        ...
        private  void ConvG16R16ToFloat32(uint* dest, ushort* src, uint size)
        {
            uint i;
            for (i = 0; i < size; i += 3)
            {
        ...
        private  void ConvR16ToFloat32(uint* dest, ushort* src, uint size)
        {
            uint i;
            for (i = 0; i < size; i += 3)
            {
 ...
 /*       private byte[] DecompressData(DDSStruct header, byte[] data, int pixelFormat)
        {
        ...
        private  byte[] DecompressDXT1(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            ...
            int rr = 0;
                for (int z = 0; z < depth; z++)
                {
                    for (int y = 0; y < height; y += 4)
                    {
                        ...
                        {
                            ushort colour0 = BitConverter.ToUInt16(data, rr); rr += 2;
                            ushort colour1 = BitConverter.ToUInt16(data, rr); rr += 2;
                    DxtcReadColor(colour0, ref colours[0]);
                            DxtcReadColor(colour1, ref colours[1]);

                            uint bitmask = BitConverter.ToUInt32(data, rr); rr += 4;
        ...
        private byte[] DecompressDXT2(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int width = (int)header.width;
            int height = (int)header.height;
        ...
        private  byte[] DecompressDXT3(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * PixelFormatToBpc(pixelFormat));
                ...
                for (int z = 0; z < depth; z++)
                {
                    for (int y = 0; y < height; y += 4)
                    {
                        for (int x = 0; x < width; x += 4)
                        {
                            ...
                            Buffer.BlockCopy(data, rr, alpha, 0, 8);                    
                            rr += 8;
        ...
        private byte[] DecompressDXT4(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int width = (int)header.width;
            int height = (int)header.height;
        ...
        private  byte[] DecompressDXT5(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * PixelFormatToBpc(pixelFormat));
        ...
        private  byte[] DecompressRGB(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
        ...
        private  byte[] DecompressRGBA(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        private  byte[] Decompress3Dc(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        private  byte[] DecompressAti1n(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        private  byte[] DecompressLum(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        private  byte[] DecompressRXGB(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        private byte[] DecompressFloat(DDSStruct header, byte[] data, int pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(this.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
        ...
        public const int RGB = 1;
            /// <summary>
            /// 16-bit DXT-1 compression = 0; 1-bit alpha.
            /// </summary>
        public const int DXT1 = 2;
            /// <summary>
            /// DXT-2 Compression
            /// </summary>
        public const int DXT2 = 3;
            /// <summary>
            /// DXT-3 Compression
            /// </summary>
        public const int DXT3 = 4;
            /// <summary>
            /// DXT-4 Compression
            /// </summary>
        public const int DXT4 = 5;
            /// <summary>
            /// DXT-5 Compression
            /// </summary>
        public const int DXT5 = 6;
            ...
            /// 3DC Compression
            /// </summary>
        public const int THREEDC = 7;
            /// <summary>
            /// ATI1n Compression
            /// </summary>
        public const int ATI1N = 8;
            public const int LUMINANCE = 9;
            public const int LUMINANCE_ALPHA = 10;
            public const int RXGB = 11;
            public const int A16B16G16R16 = 12;
            public const int R16F = 13;
            public const int G16R16F = 14;
            public const int A16B16G16R16F = 15;
            public const int R32F = 16;
            public const int G32R32F = 17;
            public const int A32B32G32R32F = 18;
            /// <summary>
            /// Unknown pixel format.
            /// </summary>
        public const int UNKNOWN = 19;
    }
        #endregion
